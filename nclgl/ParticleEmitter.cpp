...
ParticleEmitter::ParticleEmitter(void)	{
	...
	*/
	...
	texture = SOIL_load_OGL_texture(TEXTUREDIR"doge.png",SOIL_LOAD_AUTO,SOIL_CREATE_NEW_ID,SOIL_FLAG_COMPRESS_TO_DXT);
}
...
*/
	...
	p->colour		= Vector4(
		dis(generator) * 0.5f + 0.5f,
		dis(generator) * 0.5f + 0.5f,
		dis(generator) * 0.5f + 0.5f,
		1.0);

	p->direction	= initialDirection;
	p->direction.x += (dis(generator) * particleVariance);
	p->direction.y += (dis2(generator) * particleVariance);
	p->direction.z += (dis(generator) * particleVariance);
