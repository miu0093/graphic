#include "Renderer.h"
 Renderer::Renderer(Window & parent) : OGLRenderer(parent) {
	 ...
	 meshes[0]->texture2 = SOIL_load_OGL_texture(TEXTUREDIR"brick.tga", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
	 meshes[1] -> SetTexture(SOIL_load_OGL_texture(TEXTUREDIR"stainedglass.tga",SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0));
	 meshes[1]->texture2 = SOIL_load_OGL_texture(TEXTUREDIR"sample2.png", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
	...
	currentShader = new Shader(SHADERDIR"VertexMultiTex.glsl",SHADERDIR"FragMultiTex.glsl");
	//brickShader
 ...
 void Renderer::RenderScene() {
	 ...
	 for (unsigned int i = 0; i < 2; ++i) {
		 ...
		 glUniformMatrix4fv(glGetUniformLocation(
			 ...
			 currentShader->GetProgram(), "modelMatrix"), 1, false, (float *)& Matrix4::Translation(positions[i]));
